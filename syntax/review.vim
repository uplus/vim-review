...
syn match reviewColumn    "^=\+\[\(column\|nodisp\|notoc\)\]\s\+.\+"
...
syn region reviewList   start="^//list\(num\)\?\[.*\]\[.*\]{" end="^//}"
syn region reviewEmlist start="^//emlist\(num\)\?{" end="^//}"
syn region reviewCmdCap start="^//cmd{" end="^//}"
syn region reviewQuote  start="^//quote{" end="^//}"
syn region reviewImage  start="^//\(indep\|numberless\)\?image\[.*\]\[.*\]{" end="^//}"
